...
static mv_seq_t* scale_space_extrema(mv_image_t***, int, int, double, int,
    mv_mem_storage_t*);
...
static void interp_step(mv_image_t***, int, int, int, int, double*, double*,
    double*);
static mv_matrix_t* deriv_3D(mv_image_t***, int, int, int, int);
static mv_matrix_t* hessian_3D(mv_image_t***, int, int, int, int);
...
static void calc_feature_scales(mv_seq_t*, double, int);
static void adjust_for_img_dbl(mv_seq_t*);
static void calc_feature_oris(mv_seq_t*, mv_image_t***);
...
static void add_good_ori_features(mv_seq_t*, double*, int, double,
struct feature*);
static struct feature* clone_feature(struct feature*);
static void compute_descriptors(mv_seq_t*, mv_image_t***, int, int);
...
/**
   ...
   @see _sift_features()
   */
int sift_features(mv_image_t* img, struct feature** feat)
{
...
/**
   ...
   @see sift_keypoints()
   */
int _sift_features(mv_image_t* img, struct feature** feat, int intvls,
    double sigma, double contr_thr, int curv_thr,
    int img_dbl, int descr_width, int descr_hist_bins)
{
    mv_image_t* init_img;
    mv_image_t*** gauss_pyr, *** dog_pyr;
    mv_mem_storage_t* storage;
    mv_seq_t* features;
    int octvs, i, n = 0;
    ...
    *feat = (struct feature*)mv_cvt_seq_2_array(features, *feat, CV_WHOLE_SEQ);
    ...
    mv_release_mem_storage(&storage);
...
static mv_image_t* convert_to_gray32(mv_image_t* img)
{
    ...
    else
    {
        ...
        mv_cvt_color(img, gray8, CV_BGR2GRAY);
    }
...
/*
  ...
  array
  */
static mv_image_t*** build_gauss_pyr(mv_image_t* base, int octvs,
    int intvls, double sigma)
{
    mv_image_t*** gauss_pyr;
    static const int MAX_INTVLS = 128;
    const int _intvls = intvls;
    assert(_intvls <= MAX_INTVLS);
    double sig[MAX_INTVLS + 3], sig_total, sig_prev, k;
    int i, o;

    gauss_pyr = (mv_image_t***)calloc(octvs, sizeof(mv_image_t**));
    for (i = 0; i < octvs; i++)
        gauss_pyr[i] = (mv_image_t **)calloc(intvls + 3, sizeof(mv_image_t *));
    ...
    {
        if (o == 0 && i == 0)
            gauss_pyr[o][i] = mv_clone_image(base);
...
/*
  ...
  */
static mv_image_t* downsample(mv_image_t* img)
{
...
/*
  ...
  */
static mv_image_t*** build_dog_pyr(mv_image_t*** gauss_pyr, int octvs, int intvls)
{
    mv_image_t*** dog_pyr;
    int i, o;

    dog_pyr = (mv_image_t***)calloc(octvs, sizeof(mv_image_t**));
    for (i = 0; i < octvs; i++)
        dog_pyr[i] = (mv_image_t**)calloc(intvls + 2, sizeof(mv_image_t*));
...
/*
  ...
  */
static mv_seq_t* scale_space_extrema(mv_image_t*** dog_pyr, int octvs, int intvls,
    ...
    mv_mem_storage_t* storage)
{
    ...
    features = mv_create_seq(0, sizeof(mv_seq_t), sizeof(struct feature), storage);
    for (o = 0; o < octvs; o++)
    for (i = 1; i <= intvls; i++)
...
/*
  ...
  */
static int is_extremum(mv_image_t*** dog_pyr, int octv, int intvl, int r, int c)
{
...
/*
  ...
  */
static struct feature* interp_extremum(mv_image_t*** dog_pyr, int octv,
...
static void interp_step(mv_image_t*** dog_pyr, int octv, int intvl, int r, int c,
    double* xi, double* xr, double* xc)
{
    mv_matrix_t* dD, *H, *H_inv, X;
    double x[3] = { 0 };
...
/*
  ...
  */
static mv_matrix_t* deriv_3D(mv_image_t*** dog_pyr, int octv, int intvl, int r, int c)
{
    mv_matrix_t* dI;
    double dx, dy, ds;
...
/*
  ...
  \ Ixs  Iys  Iss /
  */
static mv_matrix_t* hessian_3D(mv_image_t*** dog_pyr, int octv, int intvl, int r,
    int c)
{
    mv_matrix_t* H;
...
/*
  ...
  */
static double interp_contr(mv_image_t*** dog_pyr, int octv, int intvl, int r,
...
/*
  ...
  */
static int is_too_edge_like(mv_image_t* dog_img, int r, int c, int curv_thr)
{
...
static struct feature* get_feature_seq(mv_seq_t* features, int index)
{
    assert(sizeof(features->first[0]) == sizeof(mv_seq_block_t));
    assert(features->elem_size == sizeof(struct feature));

    if (features->first && index < features->first->count) {
        return (struct feature*)(features->first->data + index * sizeof(struct feature));
    }
    else {
        return (struct feature*)mv_get_seq_elem(features, index);
    }
}
...
/*
  ...
  */
static void calc_feature_scales(mv_seq_t* features, double sigma, int intvls)
{
    struct feature* feat;
    struct detection_data* ddata;
    double intvl;
    ...
    for (i = 0; i < n; i++)
    {
        feat = get_feature_seq(features, i);
        ddata = feat_detection_data(feat);
...
/*
  ...
  @param features array of features
  */
static void adjust_for_img_dbl(mv_seq_t* features)
{
    struct feature* feat;
    int i, n;
...
/*
  ...
  @param gauss_pyr Gaussian scale space pyramid
  */
static void calc_feature_oris(mv_seq_t* features, mv_image_t*** gauss_pyr)
{
    struct feature* feat;
    struct detection_data* ddata;
    double* hist;
    ...
    {
        feat = (struct feature*)malloc(sizeof(struct feature));
        mv_seq_pop_front(features, feat);
        ddata = feat_detection_data(feat);
        hist = ori_hist(gauss_pyr[ddata->octv][ddata->intvl],
            ddata->r, ddata->c, SIFT_ORI_HIST_BINS,
            mv_round(SIFT_ORI_RADIUS * ddata->scl_octv),
...
/*
  ...
  */
static double* ori_hist(mv_image_t* img, int r, int c, int n, int rad,
    double sigma)
{
    double* hist;
...
/*
  ...
  */
static int calc_grad_mag_ori(mv_image_t* img, int r, int c, double* mag,
    double* ori)
{
    double dx, dy;
...
/*
  ...
  */
static void add_good_ori_features(mv_seq_t* features, double* hist, int n,
...
/*
  ...
  */
static void compute_descriptors(mv_seq_t* features, mv_image_t*** gauss_pyr, int d, int n)
{
    struct feature* feat;
    struct detection_data* ddata;
    double*** hist;
...
/*
  ...
  */
static double*** descr_hist(mv_image_t* img, int r, int c, double ori,
