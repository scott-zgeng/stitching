...
public:
    ...
    void CalcFourCorner(const Matrix3d& H, mv_image_t* img2) {        
        Vector3d V1;
        Vector3d V2;        
        ...
        V2 << 0, 0, 1;
        V1 = H * V2;
        m_leftTop.x = mv_round(V1[0] / V1[2]);
        m_leftTop.y = mv_round(V1[1] / V1[2]);

        //将v2中数据设为左下角坐标
        V2 << 0, img2->height, 1;
        V1 = H * V2;
        m_leftBottom.x = mv_round(V1[0] / V1[2]);
        m_leftBottom.y = mv_round(V1[1] / V1[2]);

        //将v2中数据设为右上角坐标
        V2 << img2->width, 0, 1;
        V1 = H * V2;
        m_rightTop.x = mv_round(V1[0] / V1[2]);
        m_rightTop.y = mv_round(V1[1] / V1[2]);
        
        //将v2中数据设为右下角坐标
        V2 << img2->width, img2->height, 1;
        V1 = H * V2;
        m_rightBottom.x = mv_round(V1[0] / V1[2]);
        m_rightBottom.y = mv_round(V1[1] / V1[2]);
    }

private:
    mv_point_t m_leftTop;
    mv_point_t m_leftBottom;
    mv_point_t m_rightTop;
    mv_point_t m_rightBottom;;
};
...
int image_stitching_entry()
{
    ...
    Matrix3d H;
    int ret = ransac_module::instance()->process(&features2, H);
    if (ret != 0) {
